...
import (
	...
	clientset "crddemo/pkg/client/clientset/versioned"
	informers "crddemo/pkg/client/informers/externalversions"
)

// 程序启动参数
var (
	flagSet              = flag.NewFlagSet("crddemo", flag.ExitOnError)
	master               = flag.String("master", "", "The address of the k8s API server. Overrides any value in kubeconfig. Only required if out-of-cluster.")
...
func main() {
	...
	if err != nil {
		glog.Fatalf("Error building kubecondig: %s", err.Error())
	}
	...
	// 生成一个 crddemo 组的 Mydemo 对象传递给自定义控制器
